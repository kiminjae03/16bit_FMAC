...
int main()
{
    ...
    strftime(output_filename, sizeof(output_filename), "05_mul_denormal_grs_output_%y%m%d_%H%M%S_.txt", t);
    ...
    printf(BLUE  "##  MULTIPLIER DENORMAL GRS              VERSION : %s  ##\n" RESET, _MULTI_DENORMAL_GRS_VERSION_);
    printf(CYAN  "################################################################\n\n" RESET);
    // fprintf(output_file, "\n");
    fprintf(output_file, "################################################################\n");
    fprintf(output_file, "##  FLOATING POINT                                            ##\n");
    fprintf(output_file, "##  MULTIPLIER DENORMAL GRS              VERSION : %s  ##\n", _MULTI_DENORMAL_GRS_VERSION_);
    fprintf(output_file, "################################################################\n\n");

    for(int i=0; i<46491; i++) {
    // for(int i=0; i<20; i++) {
